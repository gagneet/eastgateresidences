...
function setupDevServer(config) {
    config.setupMiddlewares = (middlewares, devServer) => {
        ...
        const isAllowedOrigin = (origin) => {
            ...
            // Allow eastgate.gagneet.com domain
            if (origin.match(/^https:\/\/(www\.)?eastgate\.gagneet\.com$/)) {
                return true;
            }
            ...
            try {
                ...
                for (const change of variableEditChanges) {
                    ...
                    if (result.success) {
                        ...
                        try {
                            execSync(`git -c user.name="visual-edit" -c user.email="admin@eastgate.gagneet.com" add "${result.file}"`);
                            execSync(`git -c user.name="visual-edit" -c user.email="admin@eastgate.gagneet.com" commit -m "visual_edit_variable_${timestamp}"`);
                        } catch (gitError) {
                ...
                Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
                    ...
                    try {
                        ...
                        execSync(`git -c user.name="visual-edit" -c user.email="admin@eastgate.gagneet.com" add "${targetFile}"`);
                        execSync(`git -c user.name="visual-edit" -c user.email="admin@eastgate.gagneet.com" commit -m "visual_edit_${timestamp}"`);
                    } catch (gitError) {
