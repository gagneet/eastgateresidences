// @featuretrace:levy — Generic CSV upload form (budget actuals, fund balances, and other finance imports).
// Layer: frontend
// Data flow: this page -> POST /finance/upload-budget-actuals | /finance/fund-balances | other
//            endpoint prop -> backend/routers/finance.py (building-scoped).
// Related: backend/routers/finance.py
//           frontend/src/app/(app)/admin/financial-data/page.tsx
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-4 text-sm text-muted-foreground">
                                <span>Planned: <strong
                                    className="text-foreground">{formatMoneyFromDollars(totalPlanned)}</strong></span>
                                <span>Actual YTD: <strong
                                    className="text-foreground">{formatMoneyFromDollars(totalActual)}</strong></span>
                                {overBudget > 0 && (
                                ...
                                <TableBody>
                                    {preview.map((row, i) => (
                                        <TableRow key={i}
                                            ...
                                            <TableCell
                                                className="text-right text-sm font-mono">{formatMoneyFromDollars(row.planned)}</TableCell>
                                            <TableCell
                                                className="text-right text-sm font-mono">{formatMoneyFromDollars(row.actual)}</TableCell>
                                            <TableCell className="text-right text-sm font-mono">
                                                {varianceIcon(row.variance)}{formatMoneyFromDollars(Math.abs(row.variance))}
                                            </TableCell>
                                            <TableCell
                                                className="text-right text-sm font-mono text-muted-foreground">{formatMoneyFromDollars(row.previous_actual)}</TableCell>
                        ...
                        <div className="grid grid-cols-3 gap-3 p-4 rounded-lg bg-muted/40 border">
                            <div className="text-center">
                                ...
                                <p className="text-lg font-semibold text-primary">
                                    {formatMoneyFromDollars(parseFloat(form.admin_balance) || 0)}
                                </p>
                            </div>
                            <div className="text-center">
                                <p className="text-xs text-muted-foreground">Sinking Fund</p>
                                <p className="text-lg font-semibold text-primary">
                                    {formatMoneyFromDollars(parseFloat(form.sinking_balance) || 0)}
                                </p>
                            </div>
                            <div className="text-center border-l">
                                <p className="text-xs text-muted-foreground">Total</p>
                                <p className="text-lg font-bold">{formatMoneyFromDollars(totalBalance)}</p>
                            </div>
                        </div>
                    )}
                ...
                {result && (
                    <Alert variant={result.success ? 'default' : 'destructive'} className="mt-4">
                        ...
                        <AlertDescription>
                            {result.success ? (
                                <div className="text-sm space-y-1">
                                    <p>Admin Fund: <strong>{formatMoneyFromDollars(result.data.admin_balance)}</strong></p>
                                    <p>Sinking Fund: <strong>{formatMoneyFromDollars(result.data.sinking_balance)}</strong></p>
                                    <p>Total: <strong>{formatMoneyFromDollars(result.data.total)}</strong></p>
...
/**
 * Generic CSV upload page for finance data that doesn't go through the Financial Year
 * Import wizard — budget actuals and fund balance snapshots. Renders a rich CSV preview
 * (DropZone + parsed table) before POSTing to the endpoint supplied by the caller.
 */
const FinancialDataManagementPage = () => {
    const {user} = useAuth();

    // Must match backend/routers/finance.py's allowed_roles for both
    // /finance/upload-budget-actuals and /finance/fund-balances — a narrower list here
    // silently blocks a role the API would actually accept.
    const allowedRoles = ['super_admin', 'strata_manager', 'ec_member', 'strata_admin'];
        ...
        return (
            <div className="py-16 text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                    This page is available to Super Admin, Strata Manager, Strata Admin, and EC Members (incl. Chairman) only.
                </p>
            </div>
        );
    }
