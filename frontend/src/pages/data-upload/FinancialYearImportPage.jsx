// @featuretrace:levy — Financial Year CSV bulk-import UI: unit owners, annual levy, budget categories, unit levy status.
// Layer: frontend
// Data flow: this page -> GET /financial-import/templates/{type}, GET /financial-import/history,
//            POST /financial-import/{sheet_type} -> backend/routers/financial_import.py (building-scoped).
// Related: backend/routers/financial_import.py
//           backend/services/financial_import_service.py
//           frontend/src/app/(app)/admin/financial-year-import/page.tsx
...
// Must match backend/routers/financial_import.py's _ALLOWED_ROLES — a narrower list here
// silently blocks a role the API would actually accept.
const ALLOWED_ROLES = ['super_admin', 'strata_manager', 'strata_admin'];
...
function StepDownloadTemplate({csvType, onNext, onBack, api}) {
    ...
    const handleDownload = async (format = 'csv') => {
        ...
        try {
            ...
            link.href = url;
            const downloadName = format === 'xlsx'
                ? cfg.templateName.replace(/\.csv$/, '.xlsx')
                : cfg.templateName;
        ...
        <div>
            <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">Download the template (CSV or Excel) and fill in your data</p>
                </div>
            </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="text-xs text-gray-500">Template with required columns — CSV or Excel</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                            <Button onClick={() => handleDownload('csv')} disabled={downloading} variant="outline"
                                    className="gap-2" data-testid="download-template-csv">
                                {downloading ? <Loader2 className="w-4 h-4 animate-spin"/> :
                                    <Download className="w-4 h-4"/>}
                                CSV
                            </Button>
                            <Button onClick={() => handleDownload('xlsx')} disabled={downloading} variant="outline"
                                    className="gap-2" data-testid="download-template-xlsx">
                                {downloading ? <Loader2 className="w-4 h-4 animate-spin"/> :
                                    <Download className="w-4 h-4"/>}
                                Excel
                            </Button>
...
function StepUploadFile({csvType, file, setFile, selectedYear, setSelectedYear, onNext, onBack}) {
    ...
    const handleFile = (f) => {
        // Both CSV and .xlsx are accepted; the backend normalises the workbook to
        // CSV at the upload boundary (backend/utils/tabular_upload.py).
            ...
            >
                ...
                {file ? (
                    ...
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <Badge variant="secondary" className="text-xs">.csv or .xlsx</Badge>
                    </div>
                )}
            </div>
...
/**
 * Financial Year CSV bulk-import wizard: unit owners, annual levy, budget categories,
 * and unit levy status. Multi-step flow (type select → download template → upload →
 * preview → confirm) backed by GET /financial-import/templates/{type} and
 * POST /financial-import/{sheet_type}.
 */
