# @featuretrace:arrears — Tests for OwnerUnitUpdate balance-field sync into unit_levy_ledger.
# Layer: test
# Tests: backend/server.py (/owners-units/* endpoints)
"""
...
No live DB required; all DB calls are mocked — including the cascade, see below.

`update_owner_unit` fires `asyncio.create_task(_cascade_owner_change(...))`. That task
runs AFTER the `with patch("server.db", ...)` block has exited, so it resolves the
module-level `db` at execution time — the REAL one. Until 2026-08-20 these tests
therefore wrote `strata_owners.owner = "Test Owner"` into East Gate (13195) UA042 in
production on every run, which is where that junk value kept coming back from. Every
test that calls `update_owner_unit` must patch `_cascade_owner_change` as well; the
cascade has its own coverage in test_owner_change_cascade.py.
"""
