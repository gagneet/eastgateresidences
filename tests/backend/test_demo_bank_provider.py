...
class TestIdempotency:
    ...
    async def test_same_csv_twice_returns_existing_batch(self):
        ...
        from integrations.demo_bank.ingestion import import_csv
...
class TestSignedAmounts:
    ...
    def test_syncable_filter_includes_legacy_string_dates_and_missing_status(self):
        from integrations.demo_bank.provider import _syncable_transaction_filter
    ...
    def test_csv_parse_negative_signed_amount_becomes_debit(self):
        ...
        from integrations.demo_bank.ingestion import _parse_csv_to_rows
...
class TestRoleGuards:
    ...
    def test_require_role_super_admin_passes(self):
        from routers.demo_bank import _require_role
    ...
    def test_require_role_tenant_raises_403_on_finance_endpoint(self):
        from fastapi import HTTPException
        from routers.demo_bank import _require_role, _FINANCE_ROLES
    ...
    def test_chairman_is_not_a_top_level_role(self):
        ...
        from routers.demo_bank import _FINANCE_ROLES, _MANAGER_ROLES, _ADMIN_ONLY
...
class TestCiviumGuard:
    ...
    def test_balance_only_snapshot_returns_empty_payments(self):
        ...
        from integrations.demo_bank.ingestion import _extract_strata_web_payments
...
class TestBalanceRecompute:
    ...
    async def test_recompute_skips_when_no_account(self):
        ...
        from integrations.demo_bank.ingestion import _recompute_balance
    ...
    async def test_recompute_writes_new_balance(self):
        ...
        db._db.demo_bank_transactions.aggregate = AsyncMock(return_value=agg_cursor)
...
class TestProviderName:
    ...
    def test_provider_name_constant(self):
        from integrations.demo_bank.provider import DemoBankFeed
        from integrations.demo_bank.schemas import PROVIDER
...
class TestNoLedgerWrites:
    ...
    async def test_manual_inject_does_not_write_to_levy_collections(self):
        from integrations.demo_bank.ingestion import inject_manual
        from integrations.demo_bank.schemas import ManualTransactionRequest
...
class TestIdempotencyKeyStability:
    ...
    def test_same_inputs_same_key(self):
        from integrations.demo_bank.ingestion import _idempotency_key, _external_txn_id
    ...
    def test_credit_and_debit_same_amount_different_key(self):
        ...
        from integrations.demo_bank.ingestion import _external_txn_id
...
class TestDirectWriteGate:
    ...
    async def test_strata_sync_skips_levy_ledger_when_toggle_enabled(self):
        ...
        with (
            ...
            patch("routers.strata_sync._sync_to_levy_collections", new=AsyncMock()) as mock_sync,
        ):
    ...
    def test_direct_write_toggle_name_matches_spec(self):
        ...
        from routers.strata_sync import _DIRECT_WRITE_TOGGLE
...
class TestSupportedBanksEndpoint:
    """GET /demo-bank/supported-banks — regression coverage for the schema-dir path bug.

    ingestion.py's _SCHEMA_DIR (see TestIdempotency above) and this router's own,
    independently-computed schema_dir both pointed at the pre-merge satellite package's
    sibling-directory layout (backend/data_upload/mocks/bank_schemas, which doesn't exist)
    and silently returned an empty list instead of raising — no test called this endpoint
    function directly, so it survived the 2026-08-09/10 strataos-demo-integrations merge
    undetected until a manual code audit found it. This test calls the real function (no
    mocking) so a future path regression fails loudly here instead of shipping silently.
    """

    @pytest.mark.asyncio
    async def test_returns_all_bank_schema_files_non_empty(self):
        from routers.demo_bank import list_supported_banks

        result = await list_supported_banks(current_user={"role": "owner"})

        assert result["banks"], "supported-banks list must not be empty — schema_dir path is wrong if it is"
        assert "cba" in result["banks"]
        assert "anz" in result["banks"]

    @pytest.mark.asyncio
    async def test_matches_actual_bank_schemas_directory_on_disk(self):
        from routers.demo_bank import list_supported_banks

        schema_dir = Path(__file__).resolve().parent.parent.parent / "backend" / "integrations" / "mocks" / "bank_schemas"
        expected = sorted(p.stem for p in schema_dir.glob("*.yaml"))

        result = await list_supported_banks(current_user={"role": "owner"})

        assert result["banks"] == expected
