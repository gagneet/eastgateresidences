# @featuretrace:levy — Tests for financial_import_service.py's 4 CSV processors + router endpoints.
# Layer: test
# Tests: backend/routers/financial_import.py
...
class TestModels:
    def test_import_result_defaults(self):
        from models.financial_import import ImportResult
    ...
    def test_financial_year_import_response_fields(self):
        from models.financial_import import FinancialYearImportResponse, ImportResult
    ...
    def test_csv_templates_has_all_types(self):
        from models.financial_import import CSV_TEMPLATES
...
class TestServiceHelpers:
    def test_parse_float_normal(self):
        from services.financial_import_service import _parse_float
    ...
    def test_decode_csv_utf8_bom(self):
        from services.financial_import_service import _decode_csv
...
class TestProcessUnitOwners:
    @pytest.fixture
    def mock_db(self):
        with patch("services.financial_import_service.db") as mock:
    ...
    def test_imports_three_rows(self, mock_db):
        from services.financial_import_service import process_unit_owners_csv
...
class TestProcessAnnualLevy:
    ...
    def test_imports_single_year_row(self, mock_db):
        from services.financial_import_service import process_annual_levy_csv
        result = asyncio.get_event_loop().run_until_complete(
...
class TestProcessBudgetCategories:
    ...
    def test_imports_three_categories(self, mock_db):
        from services.financial_import_service import process_budget_categories_csv
        result = asyncio.get_event_loop().run_until_complete(
...
class TestProcessUnitLevyStatus:
    ...
    def test_imports_two_rows(self, mock_db):
        from services.financial_import_service import process_unit_levy_status_csv
        result = asyncio.get_event_loop().run_until_complete(
...
class TestRowCap:
    def test_processes_at_most_500_rows(self):
        ...
        with patch("services.financial_import_service.db") as mock_db:
...
class TestRouterPermissions:
    ...
    def test_super_admin_allowed(self):
        from routers.financial_import import _check_permissions
        # Should not raise
...
class TestFileValidation:
    def test_file_over_10mb_raises_413(self):
        ...
        from routers.financial_import import _read_validated_file
    ...
    def test_xlsx_upload_is_accepted_and_transcoded_to_csv(self):
        """An Excel workbook is a first-class import format (see utils/tabular_upload.py).

        The workbook is normalised to CSV bytes at this boundary so every
        downstream process_*_csv() row processor is unchanged.
        """
        import asyncio
        import io as _io
        ...
        wb = Workbook()
        wb.active.append(["unit_number", "uoe"])
        wb.active.append(["UA001", 115])
        buf = _io.BytesIO()
        wb.save(buf)
        ...
        mock_file.filename = "owners.xlsx"

        result = asyncio.get_event_loop().run_until_complete(_read_validated_file(mock_file))
        assert result.decode("utf-8").splitlines()[0] == "unit_number,uoe"
        assert "UA001,115" in result.decode("utf-8")
...
class TestBuildResponse:
    def test_status_completed_when_no_errors(self):
        from routers.financial_import import _build_response
        from models.financial_import import ImportResult
