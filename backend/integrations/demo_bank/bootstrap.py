...
async def ensure_demo_chain() -> None:
    ...
    try:
        # UPDEMO5 (StrataOS Demo Residences) is the platform demo as of 2026-08-21.
        # It replaced DEMO-0001 ("StrataOS Demo Tower"), a minimal Postgres-only shell
        # that existed only to keep the super-admin building switcher non-empty. Two demo
        # buildings served no purpose once one of them was the full sales/onboarding demo
        # with lots, owners, levies and GL accounts, so the Tower was retired and this
        # bootstrap now seeds the useful one.
        #
        # seeds.demo_customer is idempotent (uuid5-derived keys), so re-running it against
        # an existing demo updates in place rather than duplicating.
        # `seed` and NOT `main`: main() is the argparse entry point and calls
        # asyncio.run() itself, which raises inside the already-running FastAPI
        # event loop. seed() is the awaitable.
        from seeds.demo_customer import seed as _seed_demo_chain
    except Exception as exc:  # noqa: BLE001
