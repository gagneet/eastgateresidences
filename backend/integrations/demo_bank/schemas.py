...
SourceType = Literal[
    ...
    # A live BankFeedProvider connection — Basiq, Frollo, a direct bank feed, or a
    # trust-account feed. Distinct from "csv_upload" (a statement a human exported and
    # uploaded) because the platform pulled it from the institution itself, and distinct
    # from every reconstruction value because nothing about it was modelled.
    # Per the 2026-08-27 intake contract, such a feed MATERIALISES rows here rather than
    # being routed straight to the GL: Demo Bank is the only door.
    "bank_feed",
]
...
class DemoBankAccountCreate(BaseModel):
    """Body for POST /demo-bank/accounts — registers a new emulated bank account for a building."""
    ...
    # The institution's own identifier for the account behind a live feed, so an incoming
    # Basiq/Frollo/direct/trust transaction routes to the right ledger. NOT unique across
    # the platform: NSW permits one agent trust account to hold money for several schemes
    # provided each has its own ledger (Property and Stock Agents Act 2002 s86), while ACT
    # requires a per-scheme account (UTMA 2011 Pt 5 Div 5.1). One row per scheme satisfies
    # both; a unique index here would reject a lawful NSW arrangement.
...
class DemoBankAccountResponse(BaseModel):
    """Full account record returned by GET/POST /demo-bank/accounts endpoints."""
...
class DemoBankBalanceResponse(BaseModel):
    """Lightweight current-balance snapshot for a single Demo Bank account."""
...
class DemoBankTransactionResponse(BaseModel):
    """A single staged Demo Bank transaction, including optional historical-reconstruction
    provenance fields (transaction_origin, reconstruction_batch_id, etc.) that are only
    populated on rows materialised from an approved ReconstructionManifest."""
...
class TransactionListResponse(BaseModel):
    """Paginated envelope for GET /demo-bank/accounts/{account_ref}/transactions."""
...
class ImportBatchResponse(BaseModel):
    """Audit-log record for one CSV/Strata-Web import run (one row per POST /demo-bank/import/*)."""
...
class ImportResultResponse(BaseModel):
    """Immediate response returned to the caller of a CSV/Strata-Web import request."""
