...
from integrations.demo_bank.schemas import (
    PROVIDER,
    ImportStatus,
...
async def ingest_provider_feed(
    db,
    building_id: str,
    account_ref: str,
    transactions: Iterable[Any] | AsyncIterator[Any],
    *,
    provider_name: str,
    is_test_data: bool = False,
) -> dict[str, int]:
    """Materialise a live bank feed into Demo Bank. The door every real feed comes through.

    Accepts ``BankTxObserved`` envelopes from ANY ``BankFeedProvider`` — Basiq, Frollo, a
    direct bank connection, a trust-account feed — and writes them as ordinary Demo Bank
    transactions. From that point they are indistinguishable in handling from any other
    staged row: the GL pulls from here, matching and allocation run unchanged.

    Why this exists (intake contract, operator decision 2026-08-27, recorded in CLAUDE.md
    and rules/post-compact-critical.md §15): a provider integration is an input ADAPTER to
    Demo Bank, not a parallel path into the GL. Before this function the module could only
    act AS a provider — ``provider.py`` reads these rows OUT and emits envelopes. Nothing
    could bring a real feed IN, so a Basiq connector had nowhere to land except the GL,
    which is precisely the bypass the contract forbids.

    Provenance, and why a bank feed is tagged differently from everything else here:

    * ``source_type="bank_feed"`` — the platform pulled this from the institution.
      Distinct from ``csv_upload``, which is a statement a human exported by hand.
    * ``transaction_origin="observed_bank_feed"`` — an EXISTING value, not a new one.
      It already means "StrataOS observed this at a real bank" as opposed to modelled it.
    * ``provenance_class="observed"`` — the counterpart to ``"reconstruction"``, which is
      what every modelled row carries.
    * ``requires_review=False`` — this is the reconciliation answer. A reconstructed row
      needs review because the platform inferred it; a bank-observed row arrives with the
      institution's own proof behind it, so it does not re-enter the evidence queue.
      It still goes through matching and allocation like everything else — "proven to have
      happened" is not the same claim as "known which lot it belongs to".

    Idempotency is the provider's transaction id, carried into ``external_transaction_id``
    and through the same ``_idempotency_key`` every other path uses. Re-pulling an
    overlapping window therefore updates rather than duplicates, which matters because
    bank feeds are routinely re-fetched for a rolling period.

    The envelope's ``amount_cents`` is SIGNED (positive credit, negative debit) while this
    collection stores an absolute amount plus a ``direction``. The conversion happens here,
    at the boundary, exactly mirroring ``provider.py`` on the way out.

    Returns counts of ``{"ingested", "updated", "skipped"}``.
    """
    stats = {"ingested": 0, "updated": 0, "skipped": 0}

    # Accept either a plain iterable or the AsyncIterator the Protocol specifies, so a
    # caller can pass a provider's stream directly or a pre-collected list from a test.
    ...
    async for tx in stream:
        signed = int(getattr(tx, "amount_cents", 0))
        if signed == 0:
            # A zero-amount line carries no financial effect and cannot be given a
            # direction. Skipped rather than stored with an arbitrary one.
            stats["skipped"] += 1
            continue

        occurred = getattr(tx, "occurred_at", None) or _now()
        created = await _upsert_transaction(
            db,
            building_id=building_id,
            account_ref=account_ref,
            source_type="bank_feed",
            source_batch_id=None,
            is_test_data=bool(getattr(tx, "is_test_data", False) or is_test_data),
            posted_date=occurred,
            effective_date=occurred,
            amount_cents=abs(signed),
            direction="credit" if signed > 0 else "debit",
            description=getattr(tx, "description", "") or "",
            reference=getattr(tx, "bpay_crn", None) or getattr(tx, "osko_e2e_id", None),
            payer_name=None,
            payment_channel=_infer_channel(getattr(tx, "description", "") or ""),
            running_balance_cents=getattr(tx, "balance_after_cents", None),
            confidence="high",
            provenance_class="observed",
            evidence_type="bank_feed",
            requires_review=False,
            transaction_origin="observed_bank_feed",
            # lot_ref_raw is deliberately NOT promoted to unit_number: a free-text bank
            # description is a hint for the matching engine, never a resolved lot. Passing
            # it here would let a regex masquerade as a confirmed attribution.
        )
        stats["ingested" if created else "updated"] += 1

    if stats["ingested"] or stats["updated"]:
        await _recompute_balance(db, building_id, account_ref)

    logger.info(
        "demo bank: ingested live feed from %s for %s/%s — %s new, %s updated, %s skipped",
        provider_name, building_id, account_ref,
        stats["ingested"], stats["updated"], stats["skipped"],
    )
    return stats
...
async def ensure_account(
    ...
    ``provider_account_id`` is the institution's OWN identifier for the account behind a
    live feed (Basiq/Frollo/direct/trust), so an incoming transaction can be routed to
    the right ledger. Optional: reconstruction, CSV and manual accounts have no upstream.

    It is deliberately NOT unique across the platform, and the reason is legal rather
    than technical. Confirmed against legislation 2026-08-27:

      * ACT — Unit Titles (Management) Act 2011 Pt 5 Div 5.1 requires the owners
        corporation to hold its OWN bank account. One account per Unit Plan.
      * NSW — Strata Schemes Management Act 2015 s58 defers to the Property and Stock
        Agents Act 2002 s86: a managing agent's trust money sits in a regulated trust
        account and the agent must keep a separate trust account LEDGER per owners
        corporation. Segregation is mandated at the LEDGER level, so one real bank
        account may lawfully hold money for several schemes.

    So two schemes CAN legitimately share a provider_account_id under NSW pooling. A
    unique index on it would reject the first compliant NSW agent managing two schemes on
    one trust account. Uniqueness stays on (building_id, account_ref) — one Demo Bank
    account row per scheme, which satisfies ACT's separate-account rule and simultaneously
    gives NSW its per-scheme ledger.
    """
    now = _now()
    await db._db.demo_bank_accounts.update_one(
        {"building_id": building_id, "account_ref": account_ref},
    ...
    # $setOnInsert above only fires on creation, so an account registered before a feed
    # was connected would never learn its provider id. Applied separately, and only when
    # supplied, so a later manual/CSV call cannot blank a live mapping.
    if provider_account_id:
        await db._db.demo_bank_accounts.update_one(
            {"building_id": building_id, "account_ref": account_ref},
            {"$set": {"provider_account_id": provider_account_id,
                      "provider_name": provider_name, "updated_at": now}},
        )
    return account_ref
