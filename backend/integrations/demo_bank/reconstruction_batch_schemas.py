...
class ReconstructionBatch(BaseModel):
    ...
    def is_manifest_locked(self) -> bool:
        """True once the batch has an approved/materialised manifest that must not be regenerated."""
        return self.status in MANIFEST_LOCKED_STATUSES

    def is_terminal(self) -> bool:
        """True if the batch has reached a final status (no further status transition is valid)."""
...
class YearFundReconciliationLine(BaseModel, frozen=True):
    """Reconciliation check row: expected vs generated levy totals for one (year, fund) pair."""
...
class LotReconciliationLine(BaseModel, frozen=True):
    """Reconciliation check row: expected vs generated levy totals for one (unit, year, fund) triple."""
...
class FundBalanceReconciliationLine(BaseModel, frozen=True):
    """Reconciliation check row: opening + reconstructed credits/debits vs expected closing balance for one account."""
