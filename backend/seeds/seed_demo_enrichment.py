"""
Demo data enrichment for Sierra (16244).

Harbourview (18932) was removed 2026-08-20; its seeding call is gone (constants remain,
unreferenced).
...
async def seed_demo_enrichment():
    ...
    print("[enrich] Starting demo data enrichment (Sierra only)...")
    ...
    # Harbourview (18932) was removed on 2026-08-20 — a synthetic seed building with no
    # users, payments or documents. This call is deliberately gone: seed_all() runs this
    # module, so restoring it would recreate the building's levies, ledger, announcements,
    # meetings, events and maintenance on the next seed.
    #
    # The HARBOUR_* constants above are left in place but are now unreferenced by this
    # entry point. They are inert data, kept so the building can be reconstructed if that
    # decision is ever reversed; delete them together with the rest if it is not.
