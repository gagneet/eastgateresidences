...
UPDEMO5) managed by Acme Strata Demo.  All records use deterministic
...
# Unit Plan number renumbered from "UP-DEMO-001" to "UPDEMO5" on 2026-08-20.
# The uuid5 identifiers above are derived from fixed strings ("acme-demo:tenant" etc.)
# and NOT from the plan number, so renumbering keeps every deterministic key stable and
# the seed stays idempotent across the change — the existing rows are updated in place
# rather than duplicated under a second identity.
ACME_BUILDING_ID    = "UPDEMO5"               # MongoDB building_id
...
async def layer_1_structural():
    ...
    async with async_session_context() as session:
        ...
        """), {"sid": ACME_SCHEME_ID, "tid": ACME_TENANT_ID, "jur": ACME_JURISDICTION,
               ...
               "legal": "The Owners — Strata Plan UPDEMO5", "abn": "00 000 000 000"})
...
async def layer_3_financial_structure():
    ...
    #
    # Mongo stores BSON dates natively, so a datetime written here comes back as a
    # datetime — while AnnualLevyResponse and most other response models contract
    # these as ISO strings. Every other writer in the tree calls .isoformat(); this
    # seed did not, and the mismatch returned HTTP 500 from GET /annual-levies for
    # the demo building on every request. Found by the owner-dashboard k6 benchmark
    # on 2026-08-26. The read path now coerces too (models/timestamps.py), but the
    # seed should still produce the same shape as everything else.
    _now_iso = _now().isoformat().replace("+00:00", "Z")
    for year_offset, year in enumerate([2025, 2026]):
        doc = {
            # uuid5-derived like every other identifier in this seed, so re-running
            # is idempotent. Its absence was the second half of the same 500:
            # AnnualLevyResponse.id is required and no other writer omits it.
            "id": _u5(f"annual-levy:{year}"),
            "building_id": ACME_BUILDING_ID,
            "year": str(year),
...
async def tear_down():
    ...
    # Postgres — discovered, not hardcoded, and committed in stages.
    #
    # Three separate traps, all of which previously let a --tear-down report success while
    # leaving the whole tenant in place:
    #
    #  1. RLS. core.lots, core.parties, core.ownership_periods and every finance.* table
    #     have NO bypass clause (CLAUDE.md footgun #7), so under the '00000000-…'
    #     sentinel a DELETE matches ZERO rows and still reports success. They must be
    #     cleared under the tenant's OWN uuid.
    #  2. Coverage. The real footprint is 19 tables / 457 rows (measured 2026-08-20) —
    #     finance.bank_transactions, finance.gl_accounts, core.feature_toggle_overrides,
    #     core.cutover_audit_log and more. The old fixed six-table list orphaned the rest.
    #  3. Transaction scope. Everything ran in ONE transaction ending with
    #     DELETE FROM core.tenants. That last statement fails while any scheme remains, and
    #     the resulting rollback threw away every child delete that had just succeeded —
    #     the log said "deleted=433" and the database still held all 457 rows.
    #
    # Child rows are therefore discovered, deleted in repeated passes (letting the foreign
    # keys themselves dictate order) and COMMITTED before core.tenants is attempted at all.
    async def _clear_children() -> int:
        async with async_session_context() as session:
            await session.execute(
                text("SELECT set_config('app.tenant_id', :u, true)"), {"u": ACME_TENANT_ID}
            )
            remaining = {
                ...
                for r in (await session.execute(text("""
                    ...
                    WHERE column_name = 'tenant_id'
                      ...
                      AND NOT (table_schema = 'core' AND table_name = 'tenants')
            ...
            while remaining:
                ...
                for tbl in sorted(remaining):
                    try:
                        # Savepoint: a foreign-key error on one table must not poison the
                        # transaction for the tables that could still be cleared.
                        ...
                        last_error[tbl] = str(exc)[:200]
                        continue
                    remaining.discard(tbl)
                    progressed = True
                    if cnt:
                        deleted_total += cnt
                        log.info("  Postgres %-38s deleted=%d", tbl, cnt)
                if not progressed:
                    # Nothing moved but rows remain: a real dependency this loop cannot
                    # resolve. Report the table AND the constraint, never swallow it.
                    for tbl in sorted(remaining):
                        n = (await session.execute(
                            text(f"SELECT count(*) FROM {tbl} WHERE tenant_id = :tid"),  # noqa: S608
                            {"tid": ACME_TENANT_ID})).scalar()
                        if n:
                            log.error("  Postgres %-38s BLOCKED %d row(s): %s",
                                      tbl, n, last_error.get(tbl, "unknown"))
                    break
            await session.commit()
            return deleted_total

    deleted_total = await _clear_children()
    log.info("  Postgres child rows deleted=%d (committed)", deleted_total)

    # core.tenants last, in its own transaction and under the bypass sentinel (its policy
    # has one). Kept separate so a failure here cannot undo the child deletes above.
    async with async_session_context() as session:
        await session.execute(
            text("SELECT set_config('app.tenant_id', :u, true)"), {"u": _BYPASS_UUID}
        )
        try:
            ...
            log.error("  Postgres core.tenants NOT deleted — %s", str(exc)[:200])

    log.info("Tear-down complete.")
    # KNOWN LIMITATION — a tear-down cannot remove posted GL rows.
    # finance.journal_lines / journal_entries carry immutability triggers
    # (trg_prevent_posted_line_update -> prevent_posted_line_mutation) that reject DELETE
    # on anything belonging to a POSTED entry: "journal line <id> belongs to a posted
    # entry and cannot be modified or deleted." That is the accounting invariant working
    # as intended — posted journals are reversed, never deleted — and strataos_user is not
    # a superuser, so session_replication_role cannot switch it off.
    # Consequence: once this demo has posted entries, finance.journal_lines,
    # journal_entries, gl_accounts, funds, accounting_periods, core.schemes and
    # core.tenants all survive the tear-down, and the errors above are expected rather
    # than a script fault. Re-running --seed is safe: every identifier is uuid5-derived,
    # so the surviving rows are updated in place. To genuinely retire the demo GL, reverse
    # the entries through the finance service (reverse_entry) rather than deleting them.
...
async def seed(layers: list[int] | None = None):
    log.info("=== Acme Strata Demo seed (UPDEMO5) ===")
