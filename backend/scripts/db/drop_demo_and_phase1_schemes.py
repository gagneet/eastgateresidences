...
Safety:
    ...
    - Never touches UPDEMO5 (StrataOS Demo Residences / Acme demo; "UP-DEMO-001"
  before the 2026-08-20 renumber — both ids stay protected).
...
# Both the pre- and post-2026-08-20 demo plan numbers are protected: an environment
# that has not been renumbered yet still matches the old id, and one that has still
# matches the new. Protecting an extra id is harmless; missing one deletes the demo.
PROTECTED = {"13195", "UP-DEMO-001", "UPDEMO5"}
    ...
    rows = await session.execute(
        text(
            ...
            WHERE (scheme_name ILIKE '%Phase1 Ops%'
              ...
              AND scheme_number NOT IN ('13195', 'UP-DEMO-001', 'UPDEMO5', 'DEMO-0001')
            """
        )
    )
    ...
    candidates = await db.buildings.find(
        {
            ...
            "building_id": {"$nin": ["13195", "UP-DEMO-001", "UPDEMO5", "DEMO-0001"]},
        },
...
async def main() -> None:
    ...
    print(f"{'=' * 60}")
    print(" Protected (never touched): 13195, UP-DEMO-001, UPDEMO5, DEMO-0001")
