# @featuretrace:strata_sync — CLI entry point: interactive PIN prompt, invokes committee_report_scraper.py.
# Layer: worker
# Data flow: operator runs `python3 -m scripts.scrape_committee_report_cli` → login/PIN prompt
#            → committee_report_scraper.scrape_committee_report() → write_exports() (local files, no DB).
# Related: backend/scripts/committee_report_scraper.py
# Tests: tests/backend/test_scrape_civium_committee_report.py
