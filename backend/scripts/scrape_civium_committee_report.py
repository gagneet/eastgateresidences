#!/usr/bin/env python3
# @featuretrace:strata_sync — Fully standalone Civium scraper (Building Financials + Invoices), no app deps.
# Layer: worker
# Data flow: operator runs this file directly with only Playwright installed → login/PIN → scrape →
#            local export files. Deliberately has zero StrataOS imports — a portable diagnostic/backup tool,
#            not part of the in-app sync flow (see run_scraper.py for that).
# Related: backend/scripts/run_scraper.py (in-app equivalent, independently maintained)
"""Standalone Civium committee-report scraper (Building Financials + Invoices).

Self-contained: needs ONLY Playwright + a Chromium build. No StrataOS backend,
no MongoDB, no app package on sys.path. Drop this single file anywhere and run it.

WHAT IT DOES
    1. Logs in to https://my.civiumstrata.com.au/login.aspx (email + password).
    2. Prompts for the one-time PIN with hidden terminal input (you type it when
       Civium sends it), then verifies.
    3. Opens https://my.civiumstrata.com.au/committeerpt.aspx.
    4. Building Financials tab: expands every collapsed caret/triangle row, then
       reads every visible row (parent summaries + expanded detail rows).
    5. Invoices tab: iterates through ALL pagination pages, reading every row.
    6. Writes a lossless JSON snapshot + one UTF-8 CSV per tab.

SETUP (once, on the server)
    python3 -m venv .venv && source .venv/bin/activate
    pip install playwright
    playwright install chromium        # skip if PLAYWRIGHT_BROWSERS_PATH is pre-provisioned

RUN
    # credentials via env (recommended) — PIN is always prompted interactively:
    export PORTAL_EMAIL="committee@example.com"
    export PORTAL_PASSWORD="..."
    python3 scrape_civium_committee_report.py --output-dir ./civium-export

    # or omit the env vars and the script prompts for them (password hidden):
    python3 scrape_civium_committee_report.py --output-dir ./civium-export

    # headless only AFTER you've confirmed the selectors match the live portal:
    python3 scrape_civium_committee_report.py --output-dir ./civium-export --headless

NOTES
    * Headed (visible) Chromium is the DEFAULT so you can watch the login/PIN
      flow and spot selector drift; use --headless for unattended runs.
    * Read-only: this script never writes to any StrataOS datastore. It only
      writes the JSON/CSV files under --output-dir.
    * If login/PIN selectors ever drift (Civium markup change), run headed and
      adjust the *_SELECTORS lists near the login flow — everything else is
      structure-agnostic (it reads whatever <table> rows are visible).
"""
...
# --------------------------------------------------------------------------- #
# Pure helpers (unit-testable; no browser, no I/O)
# --------------------------------------------------------------------------- #
...
async def expand_all_rows(page, *, max_controls: int = 500) -> int:
    ...
    are collapsed. A bounded fallback handles Civium's small caret image when no
    semantic state is present. Each control is marked in the DOM before clicking
    so client-side redraws cannot click it twice; server postbacks naturally
    replace the DOM and are handled by the next pass.
    """
    selectors = [
        "[aria-expanded='false']",
        ".collapsed:visible",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scrape Civium Building Financials (expanded) and all Invoice pages",
    )
