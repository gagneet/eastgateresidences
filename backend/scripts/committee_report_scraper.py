# @featuretrace:strata_sync — Playwright extraction helpers for Civium's committee report pages (no DB writes).
# Layer: service
# Data flow: scrape_committee_report_cli.py / scrape_civium_committee_report.py → scrape_committee_report()
#            → structured rows → caller decides where to write (this module never touches Mongo/Postgres).
# Related: backend/scripts/scrape_committee_report_cli.py
#           backend/scripts/scrape_civium_committee_report.py (independent, mirrors rather than imports this)
#           backend/scripts/run_scraper.py (independent, mirrors rather than imports this)
# Tests: tests/backend/test_committee_report_scraper.py
