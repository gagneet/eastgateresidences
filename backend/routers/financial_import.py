...
from services.financial_import_service import (
...
# CSV **and** XLSX are both accepted — see backend/utils/tabular_upload.py, which
# owns the content-type allowlist. The uploaded bytes are normalised to CSV text
# before reaching the row processors, so every process_*_csv() stays format-agnostic.
...
async def _read_validated_file(file: UploadFile) -> bytes:
    """Read upload, enforce size/content-type/Magika scan, return CSV bytes.

    Accepts either a CSV or an .xlsx workbook. An .xlsx is transcoded to CSV
    bytes here — at the single upload boundary — so every downstream
    ``process_*_csv()`` row processor keeps its existing bytes-in contract and
    needs no format branching of its own.
    """
    content = await file.read()
    if len(content) > _MAX_FILE_SIZE:
        raise HTTPException(status_code=413, detail="File exceeds 10 MB limit")
    assert_content_type_allowed(file.content_type, file.filename or "")
    await scan_upload(content, context="tabular", filename=file.filename or "", max_size_bytes=_MAX_FILE_SIZE)
    return tabular_bytes_to_csv_bytes(content, filename=file.filename or "")
...
def _csv_text_to_xlsx(csv_content: str, sheet_title: str) -> bytes:
    """Render a CSV template string as an .xlsx workbook with a bolded header row.

    The header row is generated from the same CSV string the CSV download serves,
    so the two formats can never list different columns.
    """
    ...
    for row_idx, row in enumerate(reader, start=1):
        ...
        if row_idx == 1:
            for col_idx, value in enumerate(row, start=1):
                ...
                ws.column_dimensions[cell.column_letter].width = max(14, min(32, len(str(value)) + 4))
    ws.freeze_panes = "A2"
    buf = io.BytesIO()
    wb.save(buf)
    return buf.getvalue()
...
async def download_template(
    ...
    """Download a CSV or XLSX template for the given import type.

    Both formats are accepted back by the matching upload endpoint — see
    ``utils/tabular_upload.py``; ``format`` defaults to ``csv`` so existing
    ...
    if file_format == "xlsx":
        payload = _csv_text_to_xlsx(csv_content, template_type)
