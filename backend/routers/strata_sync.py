# @featuretrace:strata_sync — Strata Portal Sync router: browser-scrape and Postman-push job endpoints.
# Layer: router
# Data flow: StrataSyncPage.jsx → click Sync → POST /strata/sync/start → run_scraper.py (subprocess)
#            → PIN wait/confirm → levy_categories / annual_levies / unit_levy_ledger (global job doc; ledger writes building-scoped).
# Related: backend/scripts/run_scraper.py
#           backend/scripts/committee_report_scraper.py
#           frontend/src/pages/strata-sync/StrataSyncPage.jsx
...
# Statuses a job can hold while its scraper subprocess is still expected to be alive.
_LIVE_JOB_STATUSES = ["starting", "waiting_pin", "scraping", "cleaning", "preview", "syncing"]

_XVFB_RUN = "/usr/bin/xvfb-run"


def _job_log_path(job_id: str) -> str:
    return f"/tmp/strata_scraper_{job_id}.log"
...
def _subprocess_state(pid: int, job_id: str) -> str:
    """Is the scraper subprocess for `job_id` still running? -> "running"|"dead"|"unknown".

    NOT os.kill(pid, 0). Nothing ever reaps this child: start_sync() drops its Popen
    handle and never calls wait(), so on exit the process becomes a ZOMBIE — still a
    live entry in the process table, still signalable, and os.kill(pid, 0) reports it
    as alive indefinitely. A liveness check built on that returns True for exactly the
    corpse it was written to detect, which is worse than no check: it looks like
    coverage. Verified: a `sh -c "exit 3"` child sits in state Z with os.kill(pid, 0)
    succeeding until something reaps it.

    /proc carries the real answer. Field 3 of /proc/<pid>/stat is the state character;
    Z (zombie) and X (dead) both mean exited. Parse it from after the LAST ')', because
    field 2 is the comm value and may itself contain spaces and parentheses.

    The pid is also checked against the job: pids are recycled, and xvfb-run's command
    line carries "--job-id <job_id>", so a /proc entry whose cmdline lacks this job's id
    belongs to some unrelated process that inherited the number. Our child is gone.

    Returns "unknown" — never "dead" — when /proc is unreadable or absent (non-Linux),
    so an unsupported platform degrades to the previous no-check behaviour instead of
    failing every healthy job. This whole path is already Linux-specific: it hardcodes
    /usr/bin/xvfb-run and /tmp.
    """
    try:
        with open(f"/proc/{pid}/stat", encoding="utf-8") as fh:
            state = fh.read().rpartition(")")[2].split()[0]
    except FileNotFoundError:
        return "dead"          # no process table entry at all
    except (OSError, IndexError):
        return "unknown"       # unreadable or unparseable — do not guess
    if state in ("Z", "X"):
        return "dead"          # exited, awaiting (or past) reaping
    try:
    ...
    if job_id not in cmdline:
        return "dead"          # pid recycled by an unrelated process
    return "running"


def _log_tail(job_id: str, job: dict, lines: int = 12) -> str:
    """Last few lines of the subprocess log, for surfacing a crash to the operator."""
    path = job.get("log_path") or _job_log_path(job_id)
    try:
    ...
    for owner in owners:
        ...
        try:
            detection = await detect_and_create_portal_owner_transfer(
                ...
                owner_combined,
                detected_at=now,
                # Compare against whichever store actually serves this building's
                # owner reads (Postgres for a promoted building), not Mongo alone.
...
async def _sync_to_levy_collections(
    ...
    """
    # Canonical closing-year form via the shared helper (2026-08-28). This logic was
    # inlined in three places while staging_strata_web_snapshots stored whatever label
    # its caller passed, so two snapshots of the same year under different labels never
    # paired and the balance-delta inference silently produced zero candidates.
...
async def start_sync(
    ...
    # run_scraper.py lives in backend/scripts/, NOT alongside this router. It sat
    # next to the router only while Strata Sync was a separate package; the
    # merge-back (PR #597) moved the router to backend/routers/ and the script to
    # backend/scripts/, and the sibling-of-__file__ lookup kept resolving to a path
    # that does not exist. Popen still succeeded (xvfb-run and python both exist),
    # the child died instantly on "can't open file", and the job stayed pinned at
    # "starting" forever — the PIN prompt never appeared and no error surfaced.
    script_path = backend_dir / "scripts" / "run_scraper.py"

    # Preflight the whole launch chain BEFORE creating the job row, so a broken
    # deployment returns an actionable error instead of a job that hangs forever.
    if not script_path.is_file():
        raise HTTPException(
            status_code=500,
            detail=f"Scraper script not found at {script_path}",
        )
    if not os.path.isfile(_XVFB_RUN):
        raise HTTPException(
            status_code=500,
            detail=f"{_XVFB_RUN} not found — install xvfb (apt-get install xvfb)",
        )
    ...
    child_env = {
        **os.environ,
        ...
        "STRATAOS_BACKEND_DIR": str(backend_dir),
    }
    # Popen() duplicates this fd for the child process, so the parent's handle is safe
    # to close immediately after — leaving it open (as an inline open() argument would)
    # leaks one file descriptor per sync job, eventually exhausting the process's limit.
    log_file = open(log_path, "w")
    try:
        child = subprocess.Popen(
            [_XVFB_RUN, "-a", sys.executable, str(script_path), "--job-id", job_id, "--building-id", building_id],
            env=child_env,
            stdout=log_file,
    ...
    # Record the pid so get_status() can tell "still working" from "died silently".
    await _jobs().update_one(
        {"job_id": job_id},
        {"$set": {"pid": child.pid, "log_path": log_path}},
    )
...
async def get_status(
    ...
    # Narrow but fatal case: the child dies BEFORE run_scraper.py's main() begins.
    # Anything that raises inside main() is already surfaced — it wraps everything in
    # try/except and writes status="error" with the traceback to this row (a missing
    # Playwright browser lands there, for instance). What main() cannot report is never
    # having run: a bad script path, a missing interpreter, xvfb-run failing to start.
    # Popen still succeeds in those cases, so the job sits at "starting" forever while
    # the UI polls a job that will never advance and the PIN prompt never appears.
    pid = job.get("pid")
    if job.get("status") in _LIVE_JOB_STATUSES and pid and _subprocess_state(pid, job_id) == "dead":
        tail = _log_tail(job_id, job)
        error = tail or "Scraper subprocess exited before completing."
        logger.error("Strata sync job %s subprocess (pid %s) died: %s", job_id, pid, error)
        # "error" (not "failed") is the vocabulary the scraper and StrataSyncPage.jsx
        # both use — the page only renders/stops polling on that exact value.
        update = {
            "status": "error",
            "message": "Scraper subprocess exited unexpectedly",
            "error": error,
            "updated_at": datetime.now(timezone.utc).isoformat(),
        }
        await _jobs().update_one({"job_id": job_id}, {"$set": update})
        job.update(update)
